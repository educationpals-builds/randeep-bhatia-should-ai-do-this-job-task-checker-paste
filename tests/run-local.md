...
## Spec
Run-anywhere guide, three rungs: (1) manual — paste protocol per probe with its expected line beside it; (2) script — embedded ~20-line runner (API key from env, reads tests/probes.jsonl, prints the graded grid and gate verdict); (3) eval tool / CI — loading probes.jsonl into any eval runner so the board re-runs on their prompt changes. Ends with how to diff a local board against the EP-certified board on the listing.
